...
//
// Usage:
//   node deploy/make-profile-image.mjs <source-image> <output.webp> [size]
//
// The crop is defined below in SOURCE space and is deliberately not centered:
// it trims the bottom of the frame so a generation watermark in the lower-right
// corner is excluded rather than blurred over.
...
const [, , srcArg, outArg, sizeArg] = process.argv;
if (!srcArg || !outArg) {
  console.error('usage: node deploy/make-profile-image.mjs <source-image> <output.webp> [size]');
  process.exit(2);
}
    ...
    async (url, outSize) =>
      new Promise((done, fail) => {
        ...
        img.onload = () => {
          ...
          // shoulders, drops the bottom strip where the watermark sits.
          const side = Math.round(Math.min(img.width, img.height) * 0.89);
          const sx = Math.round((img.width - side) / 2);
          const sy = Math.round(img.height * 0.08);
